...
public sealed class Paddle
{
    ...
    private static readonly Rectangle InitialLeftPaddle = new(x: 10, y: 150, width: 20, PaddleHeight);
    ...
    public int Height => _paddle.Height;

    public int Width => _paddle.Width;

    public int X => _paddle.X;

    // ReSharper disable once UnusedMember.Global
    public int Y => _paddle.Y;
    ...
    public bool IsIntersectingWithBall(Ball ball) => _paddle.Intersects(ball.BallRectangle);
    ...
    private void InitializePaddleRectangle()
        => _paddle = Location switch
        {
            PaddleLocations.Left  => InitialLeftPaddle,
            PaddleLocations.Right => InitialRightPaddle,
    ...
    private void LimitPaddle()
    {
        ...
        if (_paddle.Y + _paddle.Height > GameOptions.WindowResolution.Y - 10)
        {
            _paddle.Y = GameOptions.WindowResolution.Y - 10 - _paddle.Height;
        }
    }
...
#pragma warning restore CA2208
    ...
    private void SimulateLeftPaddleInput(Ball ball)
    {
        ...
        int paddleCenter = _paddle.Y + _paddle.Height / 2;
    ...
    private void SimulateRightPaddleInput(Ball ball)
    {
        ...
        if (paddleCenter < ball.PositionY - 20)
        {
            _paddle.Y -= (int)((paddleCenter - ball.PositionY) * 0.08f);
        }
        else if (paddleCenter > ball.PositionY + 20)
        {
            _paddle.Y += (int)((ball.PositionY - paddleCenter) * 0.08f);
        }
    }
}
