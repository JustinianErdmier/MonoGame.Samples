...
{
    // menu itens
    private const int NumberMenuItems = 4;

    private readonly GameManager gameManager; // game manager

    private readonly string[] menuNames = new string[NumberMenuItems] { "menu_sp", "menu_mp", "menu_hp", "menu_qg" };

    private int menuSelection; // current menu selection

    private float menuTime; // menu time for animation

    private readonly ScreenManager screenManager; // screen manager

    private Texture2D textureCursorAnim; // cursor textures

    private Texture2D textureCursorArrow;

    private Texture2D textureCursorBullet;

    private Texture2D textureLens; // lens texture

    private Texture2D textureLogo; // logo texture

    // menu textures without hover
    private readonly Texture2D[] textureMenu = new Texture2D[NumberMenuItems];

    // menu textures with hover
    private readonly Texture2D[] textureMenuHover = new Texture2D[NumberMenuItems];
    ...
    public override void SetFocus(ContentManager content, bool focus)
    {
        ...
        if (focus)
        {
            ...
            textureLogo = content.Load<Texture2D>(assetName: "screens/intro_logo");
            textureLens = content.Load<Texture2D>(assetName: "screens/intro_lens");

            textureCursorAnim   = content.Load<Texture2D>(assetName: "screens/cursor_anim");
            textureCursorArrow  = content.Load<Texture2D>(assetName: "screens/cursor_arrow");
            textureCursorBullet = content.Load<Texture2D>(assetName: "screens/cursor_bullet");

            for (int i = 0; i < NumberMenuItems; i++)
            {
                textureMenu[i]      = content.Load<Texture2D>("screens/" + menuNames[i]);
                textureMenuHover[i] = content.Load<Texture2D>("screens/" + menuNames[i] + "_hover");
            }
        }
        else // loosing focus
        {
    ...
    private void DrawCursor(int x, int y)
    {
        ...
        screenManager.DrawTexture(textureCursorAnim,
                                  rect,
                                  rotation,
                                  Color.White,
        ...
        screenManager.DrawTexture(textureCursorBullet,
                                  rect,
                                  Color.White,
                                  BlendState.AlphaBlend);
        ...
        screenManager.DrawTexture(textureCursorArrow,
                                  rect,
                                  Color.White,
                                  BlendState.AlphaBlend);
    }
    ...
    public override void Draw2D(GraphicsDevice gd, FontManager font)
    {
        ...
        // screen rect
        Rectangle rect = new(gd.Viewport.X,
                             gd.Viewport.Y,
                             gd.Viewport.Width,
                             gd.Viewport.Height);

        // draw lens flare texture
        screenManager.DrawTexture(textureLens,
                                  rect,
                                  Color.White,
                                  BlendState.Additive);

        // draw logo texture
        screenManager.DrawTexture(textureLogo,
                                  rect,
                                  Color.White,
                                  BlendState.AlphaBlend);
        ...
        for (int i = 0; i < NumberMenuItems; i++)
        {
            ...
            if (i == menuSelection)
            {
                ...
                screenManager.DrawTexture(textureMenuHover[i],
                                          rect,
                                          Color.White,
                                          BlendState.AlphaBlend);
            ...
            else // item not selected
            {
                ...
                screenManager.DrawTexture(textureMenu[i],
                                          rect,
                                          Color.White,
                                          BlendState.AlphaBlend);
