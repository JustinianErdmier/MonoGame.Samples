...
public class Particle
{
    ...
    /// <summary>Create a new particle</summary>
...
{
    private readonly int count; // total number of particle

    // (when disabled will not update or render)
    private readonly DrawMode drawMode; // drawing mode (alpha or additive and glow)

    private float elapsedTime; // elapsed time since activated

    private readonly float emissionAngle; // emission cone 

    private readonly Vector4 endColor; // end particle color and opacity

    private readonly bool loop; // is a loop? (if disabled will do a single burst)

    // the list of particles
    private readonly List<Particle> particles;

    private readonly float particleTime; // particle life 

    private ParticleSystemType particleType; // particle system type

    // (random pick and end velocity is always zero)
    private readonly Vector2 pointSize; // point size range (random pick)

    // random generator
    private readonly Random random = new();

    private int renderCount; // number of particles in render buffer

    // (if less than total time particles will loop)

    private readonly Vector4 startColor; // start particle color and opacity

    private readonly Texture2D texture; // texture map for the particles

    private float totalTime; // total time for particle system

    private Matrix transform; // the particle system position and orientation

    private readonly Vector2 velocity; // start velocity range 

    // (0 for omni, >0 for more and more Z direction)

    /// <summary>Create a new particle system</summary>
    ...
    /// <summary>Get/Set the current velocity scale factor (1.0 for no scaling)</summary>
    public float VelocityScale { get; set; }

    /// <summary>Enable/Disable the particle system</summary>
    public bool Enabled { get; set; }

    /// <summary>Gets the number of particles to render (if negative tells how many particles to skip when disabled)</summary>
    public int RenderCount => Enabled ? renderCount : -renderCount;

    /// <summary>Set the particle system position and orientation</summary>
    public void SetTransform(Matrix transform)
    {
        this.transform = transform;
    }

    /// <summary>Set the total life of the particle system (set to 0 to destroy it before end)</summary>
    public void SetTotalTime(float totalTime)
    {
        this.totalTime = totalTime;
    }

    /// <summary>Create the particle systems</summary>
    private void CreateParticles()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            do
            {
                randomVelocity = new Vector3((float)random.NextDouble() - 0.5f,
            ...
            randomVelocity *= velocity.X * randomNumber + velocity.Y * (1.0f - randomNumber);

            // random scale and time offset 
            // (zero time offset to emitt all particles at same time)
            Vector2 randomVector = new((float)random.NextDouble(),
    ...
    /// <summary>Update particle systems</summary>
    ...
    /// <summary>Add the particle system to the given vertex array</summary>
    public int AddToVertArray(VertexPositionNormalTexture[] vertexBufffer,
    ...
    /// <summary>Set the effect parameters for this particle system</summary>
    public DrawMode SetEffect(EffectParameter effectWorldViewProjection,
