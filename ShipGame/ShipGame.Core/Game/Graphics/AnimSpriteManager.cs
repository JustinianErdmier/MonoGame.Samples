...
public enum DrawMode { Alpha = 0, Additive = 1, AlphaAndGlow = 2, AdditiveAndGlow = 3 }

public class AnimSpriteManager : IDisposable
{
    // linked list of active animated sprites
    private readonly LinkedList<AnimSprite> animatedSprites;

    // linked list of nodes to delete from the animated sprites list
    private readonly List<LinkedListNode<AnimSprite>> deleteSprites;

    private Effect effect; // the effect

    private EffectParameter effectFrameBlend; // effect frame blend parameter

    private EffectParameter effectFrameOffset; // effect frame offset parameter

    private EffectParameter effectFrameSize; // effect frame size parameter

    private EffectTechnique effectTechnique; // effect technique

    private EffectParameter effectTexture; // effect texture parameter

    private EffectParameter effectViewProjection; // effect view projection parameter

    private readonly VertexBuffer[] vertexBuffer; // vertex buffer for each player with 

    // all running animated sprites
    private VertexDeclaration vertexDeclaration; // vertex delcaration

    // the vertex array for all running animated sprites
    private readonly VertexPositionTexture[] vertices;

    /// <summary>Create a new animated sprite manager</summary>
    ...
    /// <summary>Add a new animated sprite</summary>
    ...
    /// <summary>Update all animated sprites</summary>
    ...
    /// <summary>Draw all sprites aligning to given camera</summary>
                     ...
                     bool           camera3rdPerson)
    {
        ...
        foreach (AnimSprite sprite in animatedSprites)
        {
            ...
            DrawMode mode = sprite.SetEffect(effectTexture,
                                             effectFrameOffset,
                                             effectFrameSize,
                                             effectFrameBlend);
            ...
            gd.DrawPrimitives(PrimitiveType.TriangleList, vertexBufferPosition, primitiveCount: 2);
                            ...
                            ContentManager content)
    {
        ...
        // load effect
        effect = content.Load<Effect>(assetName: "shaders/AnimSprite");

        // get techinque
        effectTechnique = effect.Techniques[name: "AnimSprite"];

        // get parameters
        effectTexture        = effect.Parameters[name: "Texture"];
        effectFrameOffset    = effect.Parameters[name: "FrameOffset"];
        effectFrameSize      = effect.Parameters[name: "FrameSize"];
        effectFrameBlend     = effect.Parameters[name: "FrameBlend"];
        effectViewProjection = effect.Parameters[name: "ViewProj"];
        ...
        vertexBuffer[0] = new VertexBuffer(gd,
                                           typeof(VertexPositionTexture),
                                           GameOptions.MaxSprites * 6,
                                           BufferUsage.WriteOnly);

        vertexBuffer[1] = new VertexBuffer(gd,
                                           typeof(VertexPositionTexture),
                                           GameOptions.MaxSprites * 6,
                                           BufferUsage.WriteOnly);
    }
