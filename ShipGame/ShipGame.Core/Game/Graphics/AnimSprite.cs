...
public class AnimSprite
{
    ...
    private static readonly Vector2[] QuadTexCoords = new Vector2[4] { new(x: 0, y: 0), new(x: 1, y: 0), new(x: 1, y: 1), new(x: 0, y: 1) };

    private readonly DrawMode drawMode; // drawing mode (alpha or additive and glow)

    private float elapsedTime; // elapsed time since animation start

    private readonly float frameRate; // framerate to play animation frames

    private readonly Vector2 frameSize; // frame size in X and Y directions

    private readonly int numberFrames; // number of frames in texture grid

    private readonly int numberFramesX; // number of frames per row in the texture grid

    private readonly int numberFramesY; // number of frames per column in the texture grid

    private readonly int player; // the player it is related to (-1 for no player)

    private AnimSpriteType spriteType; // animated sprite type

    private readonly Texture2D texture; // the texture grid with the animation frames

    private float totalTime; // total animation time

    private readonly float viewOffset; // view offset moves sprite in direction of camera


    /// <summary>Create a new animated sprite</summary>
    public AnimSprite(AnimSpriteType type,
                      Vector3        position,
                      float          radius,
                      float          viewOffset,
                      Texture2D      texture,
                      ...
                      int            player)
    {
        if (texture == null)
        {
            throw new ArgumentNullException(paramName: "texture");
        }
        ...
        // frame size
        float sizeX = frameSizeX / (float)texture.Width;
        float sizeY = frameSizeY / (float)texture.Height;
        ...
        totalTime   = numberFrames / frameRate;
        elapsedTime = 0;
    }


    /// <summary>The animated sprite position</summary>
    public Vector3 Position { get; set; }

    /// <summary>The animated sprite radius</summary>
    public float Radius { get; set; }

    /// <summary>Updates the animated sprite for given elapsed time and return false when animation is finished and object can be released</summary>
    ...
    /// <summary>Set the animation total time (set to zero to delete object before it is finished)</summary>
    public void SetTotalTime(float totalTime)
    {
        this.totalTime = totalTime;
    }

    /// <summary>Add the animated sprite geometry to the given vertex array aligning it to the given camera</summary>
                               ...
                               bool                    camera3rdPerson)
    {
        ...
        if (maximumVertexBufferPosition < vertexBufferPosition
            || // overflow
            maximumVertexBufferPosition >= vertexBuffer.Length) // too many
        {
            throw new ArgumentOutOfRangeException(paramName: "vertexBufferPosition");
        }
        ...
        if (camera3rdPerson || player != this.player)
        {
    ...
    /// <summary>Set the effect parameters for this animated sprite</summary>
    public DrawMode SetEffect(EffectParameter effectTexture,
                              ...
                              EffectParameter effectFrameBlend)
    {
        ...
        float opacity = Math.Min(val1: 1.0f, elapsedTime / totalTime);
        ...
        // set frame offset
        Vector4 frameOffset = new(frame     % numberFramesX,
                                  frame     / numberFramesX,
                                  nextFrame % numberFramesX,
                                  nextFrame / numberFramesX);
        ...
        // set blend factor
        float blendFactor = floatFrame - frame;
